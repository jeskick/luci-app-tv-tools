# TV-Tools 部署：停 uhttpd -> 删本插件旧文件与 LuCI 缓存 -> 上传(UTF-8/LF) -> 启动 uhttpd
...
mkdir -p /usr/lib/lua/luci/view/tv_tools /www/luci-static/tv-tools/caps /usr/bin
'@
...
Write-Host "==> chmod tv-tools adb scripts..."
ssh $Remote "chmod 755 /usr/bin/tv-tools-adb-key.sh /usr/bin/tv-tools-adb-text.sh /usr/bin/tv-tools-adb-text-inner.sh /usr/bin/tv-tools-adb-screencap.sh /usr/bin/tv-tools-adb-pkg-list.sh /usr/bin/tv-tools-adb-pkg-list-inner.sh /usr/bin/tv-tools-adb-install.sh /usr/bin/tv-tools-adb-uninstall.sh /usr/bin/tv-tools-adb-app-refresh.sh 2>/dev/null; true"
